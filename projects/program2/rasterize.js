    ...
    if (inputTriangles != String.null) { 
        ...
        for (var whichSet=0; whichSet<inputTriangles.length; whichSet++) {
            ...
            for (whichSetVert=0; whichSetVert<inputTriangles[whichSet].vertices.length; whichSetVert++) {
                var color = inputTriangles[whichSet].material;
                colorArray.push(color.diffuse[0],color.diffuse[1],color.diffuse[2],1.0);
            }
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        varying vec4 shaderColors;

        void main(void) {
            // gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0); // all fragments are white
            gl_FragColor = shaderColors;
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vertexColorAttrib = gl.getAttribLocation(shaderProgram, "colors")
