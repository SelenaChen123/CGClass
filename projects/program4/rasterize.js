...
const TEXTURE_SKY_JPG = "https://ncsucgclass.github.io/prog4/sky.jpg";
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "Backspace": // reset model transforms to default
            ...
            break;
        case "KeyB": // change mode
            if (replaceMode == "Replace") {
                replaceMode = "Modulate";
            } else {
                replaceMode = "Replace";
            }
            break;
...
function newLoadTexture(gl, url) {
            ...
            if (numLongSteps % 2 != 0)
                ...
                for (var latAngle=-latLimitAngle; latAngle<=latLimitAngle; latAngle+=angleIncr) {
                    ...
                    for (var longAngle=0; longAngle<2*Math.PI; longAngle+=angleIncr) {// for each long
                        ...
                        ellipsoidUVs.push(longAngle / (2*Math.PI), (latAngle + latLimitAngle) / Math.PI);
                    }
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                inputTriangles[whichSet].glTexture = loadTexture(gl, BASE_URL + inputTriangles[whichSet].material.texture);
            ...
            if (inputEllipsoids == String.null)
                ...
                for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
                    ...
                    ellipsoid.glTexture = loadTexture(gl, BASE_URL + ellipsoid.texture);
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            vec2 uv = vec2(1.0 - vTexturePosition.x, 1.0 - vTexturePosition.y);

            if (replaceMode) {
                gl_FragColor = texture2D(uTexture, uv);
            } else {
                gl_FragColor = vec4(colorOut, 1.0) * texture2D(uTexture, uv);
            }
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vTextureAttribLoc = gl.getAttribLocation(shaderProgram, "aTexturePosition");
                ...
                replaceModeULoc = gl.getUniformLocation(shaderProgram, "replaceMode");
...
function renderModels() {
    ...
    gl.uniform1i(replaceModeULoc, replaceMode == "Replace" ? true : false);
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        if (currSet.material.texture.indexOf("png") != -1) {
            gl.depthMask(true);
